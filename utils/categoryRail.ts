...
export type TypesGridItem = {
  title: string;
  text?: string;
  tag?: string;
};

export type TypesGridBlock = {
  type: "types-grid";
  items: TypesGridItem[];
};
...
export function isHtmlTextBlock(b: Block): b is TextBlock {
  return (
    b.type === "text" &&
    ((b as any).html === true || (b as any).format === "html")
  );
}

export function isTypesGridBlock(b: Block): b is TypesGridBlock {
  return (
    b.type === "types-grid" &&
...
export function pickExcerpt(blocks: Block[]): string | undefined {
  ...
  const bullets = blocks.find(
    (b) => b?.type === "bullets" && Array.isArray((b as any).items)
  ) as BulletsBlock | undefined;
  ...
  const types = blocks.find(
    (b) => b?.type === "types-grid" && Array.isArray((b as any).items)
  ) as TypesGridBlock | undefined;
