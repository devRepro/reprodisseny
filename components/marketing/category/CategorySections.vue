...
type SafeSection = { id: string; title: string; blocks: CategoryBlock[] };
...
const safeSections = computed<SafeSection[]>(() =>
  (props.sections || [])
    .map((section, index) => {
      ...
      return { id: makeAnchorId(rawId || title, `seccion-${index + 1}`), title, blocks };
    })
...
function isProcessBlock(
  b: CategoryBlock
): b is Extract<CategoryBlock, { type: "bullets" }> {
  return (
    b?.type === "bullets" &&
    Boolean((b as any).ordered) &&
    Array.isArray((b as any).items) &&
    (b as any).items.length === 4
  );
}

function toProcessSteps(items: string[]): ProcessStep[] {
  return (items || [])
    .map((raw, index) => {
      ...
      if (separatorIndex === -1) {
        return {
          title: `Paso ${String(index + 1).padStart(2, "0")}`,
          description: value,
        };
      }
      ...
      return {
        title: title || `Paso ${String(index + 1).padStart(2, "0")}`,
        description,
      };
    })
    .filter((step): step is ProcessStep => Boolean(step?.title && step?.description));
}
</script>

<template>
  <div v-if="safeSections.length" class="w-full relative">
    <div
      v-if="showSectionNav && safeSections.length > 1"
      class="sticky top-6 z-40 mx-auto mb-10 max-w-fit px-4 pointer-events-none flex justify-center"
    >
      <nav
        aria-label="Navegación de secciones"
        class="pointer-events-auto flex items-center gap-1.5 rounded-full border border-border/40 bg-background/80 p-1.5 shadow-sm backdrop-blur-md supports-[backdrop-filter]:bg-background/60 overflow-x-auto max-w-full no-scrollbar"
      >
        <button
      ...
      >
        ...
        <div class="space-y-8 px-6 py-8 md:px-8 md:py-10">
          ...
          >
            ...
            <ProcessSteps
              v-else-if="isProcessBlock(block)"
              title=""
              description=""
              :steps="toProcessSteps((block as any).items)"
            />
