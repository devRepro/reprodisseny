...
<template>
  <div :class="cn('space-y-8 md:space-y-10', props.class)">
    <section :aria-label="section.title || 'Detalle'" class="w-full">
      <div :class="layoutClass">
        <div class="min-w-0 space-y-6 md:space-y-8">
          <ContentDetailsSection :section="section" eyebrow="Información" :show-header="showDetailsHeader"
            :content-class="cn(
              'space-y-5 rounded-3xl border border-border/70 bg-card/75 p-5 shadow-sm md:space-y-6 md:p-7',
              'before:pointer-events-none before:block before:h-1 before:w-16 before:rounded-full before:bg-primary/70'
            )
              " />

          <aside v-if="pills.length"
            ...
            <div class="mb-3 flex items-center gap-3">
              <span class="h-px w-8 shrink-0 bg-foreground/25" aria-hidden="true" />
            ...
            <div class="flex flex-wrap gap-2">
              <AppChip v-for="pill in pills" :key="`${pill.to}-${pill.label}`" variant="related" :to="pill.to"
        ...
        <figure v-if="leadImage" class="overflow-hidden rounded-3xl border border-border/60 bg-card p-3 shadow-sm">
          <CmsImage :src="leadImage.src" :alt="leadImage.alt" width="840" height="630"
            class="aspect-[4/3] w-full rounded-[1.25rem] object-cover" />

          <figcaption v-if="leadImage.caption" class="px-2 pt-3 text-body-s leading-6 text-muted-foreground">
            {{ leadImage.caption }}
          </figcaption>
        </figure>
      </div>
    ...
    <CategoryShowcaseCta v-if="featuredProduct" :product="featuredProduct" :highlights="[
